...
これは、顧客が実際の場所データを使用してユーザーの体験を豊かにするツールのセットである「プレース」の最初のリリースです。 最初のリリースでは、主にモバイルアプリでカスタムの場所データを取得し、Adobe Experience Platform Launchを使用してそのデータを操作できるようにします。
...
Places Extensionを使用すると、モバイルアプリにPlacesライブラリを追加し、POIに基づいて作業を行うことができます。 エクスペリエンスプラットフォームの起動のルールビルダーを使用すると、ユーザーがPOIに入ってPOIを終了したときに実行するアクションをトリガーできます。
